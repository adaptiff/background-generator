  ...
  generate(width, height, configValues) {
    ...
    while (topItemsOffset < circleRadius) {
      ...
      while (leftItemsOffset < circleRadius) {
        const itemPointRadius = Math.sqrt(
          Math.pow(leftItemsOffset, 2) + Math.pow(topItemsOffset, 2)
        );
        ...
        for (let i = 0; i < 4; i++) {
          ...
          const isDuplicate =
            (leftItemsOffset === 0 && coordsModifiers[i][0] < 0) ||
            (topItemsOffset === 0 && coordsModifiers[i][1] < 0);
